...
int  train( double **trainingSamples, int *trainingLabels, int numSamples, int numFeatures)
{
    ...
    // so that your predictLabel knows whether to return NO_PTEDICTION or a valid default class label
    modelTrained = true;
...
void GreedyConstructiveSearch(void)
{
  ...
  // initialise the variables used for search e.g. workingCandidate, openList and closedList
    ...
    //  and set atGoal = goalFound() at the end of each iteration
    
    while (atGoal == false) // GENERATE AND TEST loop
    {
        // 1 - make a copy of the working candidate we can repeatedly add to
        ...
        // 2 - The GENERATE bit...
        // extend working candidate with each possible rule one at a time and add to working Candidate
        rule newrule;
        ...
        // Make a copy of the best into working Candidate
